...
#if defined(NDEBUG)
# error "Phoenix cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x737b4b4b9fdf153a8f81b8bf92c50af07ff01210c6f35f56ddf8ff234677b405");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Phoenix: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Phoenix Signed Message:\n";
...
{
    // Phoenix: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Phoenix: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Phoenix: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1520189437, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "World currency 2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04bcde7a8eb3d0138c342676569670ad80c6ac0cd25e5e8b3103441a354468b7cd7e2763ef15b22b5727b77341d93747c702bb46fee7680ed8d8d89d01620fda5e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520189437;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2350818;
        ...
        assert(block.hashMerkleRoot == uint256("0xcc11fa1665740667f86b13942366fc7d368206ac360cd19a01fcd773fd34b13f"));
...
unsigned char pchMessageStart[4] = { 0x3a, 0x55, 0x5d, 0xaa }; // Phoenix: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PhoenixMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PhoenixMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PhoenixMiner(CWallet *pwallet)
{
    printf("PhoenixMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("phoenix-miner");
    ...
    try { loop {
        ...
        printf("Running PhoenixMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PhoenixMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PhoenixMiner, pwallet));
}
